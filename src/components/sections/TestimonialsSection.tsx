...
export default function TestimonialsSection() {
  ...
  const paginate = (newDirection: number) => {
    ...
    setCurrentIndex((prevIndex) => {
      if (newDirection === 1) {
        return prevIndex === testimonialsContent.testimonials.length - 1
          ? 0
          : prevIndex + 1;
      } else {
        return prevIndex === 0
    ...
    >
      <div className="container mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative max-w-4xl mx-auto">
            ...
            <AnimatePresence initial={false} custom={direction}>
              ...
              >
                <div className="card-testimonial max-w-3xl mx-auto text-center px-4 xs:px-6">
                  <StarRating
                    rating={
                      testimonialsContent.testimonials[currentIndex].rating
                    }
                  />
          ...
          <div
            className="flex justify-center mt-6 xs:mt-8 gap-1 xs:gap-2"
            role="tablist"
              ...
              >
                <span
                  className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-2 h-2 xs:w-3 xs:h-3 rounded-full transition-all duration-300 ${
                    index === currentIndex ? "scale-125" : "bg-tc-border-300"
                  }`}
                  style={{
                    backgroundColor:
                      index === currentIndex ? "#1c1c1c" : undefined,
                  }}
                  aria-hidden="true"
                />
              </button>
